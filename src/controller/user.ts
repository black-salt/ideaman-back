  ...
  async firstPaint(ctx: Context) {
    ...
    const resPapers = papers.map(item => {
      return {
        ...
        'user': item.userId,
        'title': item.title,
        'authors': item.authors.split(',').map(item => +item),
    ...
    for await (const paper of resPapers) {
      let authorIds = paper.authors
      let res: Array<any> = await AuthorService.getAuthorList(authorIds)
      paper.authors = res.map((item: any) => item.author)
    }
